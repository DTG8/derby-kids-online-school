        ...
        <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <Link href="/" className="flex items-center gap-3 group">
              <div className="w-12 h-12 bg-[#FBBF24] rounded-xl flex items-center justify-center transform group-hover:scale-110 transition-transform">
                <GraduationCap className="w-7 h-7 text-[#1E40AF]" />
              </div>
              <div>
                <h1 className="text-xl font-bold">Derby Kids</h1>
                <p className="text-xs text-blue-200">Online School</p>
              </div>
            </Link>
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    key={link.name}
                    href={link.href}
                    className={`font-medium transition-all relative group whitespace-nowrap px-4 py-2 rounded-xl ${
                      active
                        ? "text-[#FBBF24] font-semibold bg-white/10 backdrop-blur-md border border-white/20 shadow-lg"
                        : "text-white hover:text-[#FBBF24] hover:bg-white/5 hover:backdrop-blur-sm"
                    }`}
                  >
                    {link.name}
                    {!active && (
                      <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-[#FBBF24] group-hover:w-full transition-all rounded-full"></span>
                    )}
                  </Link>
                );
              })}
              <Link href="/contact">
                <Button size="lg" className="bg-gradient-to-r from-[#F59E0B] to-[#D97706] hover:from-[#D97706] hover:to-[#F59E0B] text-white font-bold py-6 px-8 text-lg shadow-xl hover:shadow-2xl whitespace-nowrap">
                  Enroll Now
                </Button>
              </Link>
            </div>
